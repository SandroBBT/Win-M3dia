...
# ============================
# Windows 11 Troubleshooting Functions
# ============================

# Function to check available disk space
function Check-DiskSpace {
    Write-Host "Checking for sufficient disk space..."
    $disk = Get-PSDrive -Name C
    $freeSpaceGB = [math]::round($disk.Free/1GB, 2)
    $totalSpaceGB = [math]::round($disk.Used/1GB + $freeSpaceGB, 2)

    Write-Host "Total Disk Space: $totalSpaceGB GB"
    Write-Host "Free Disk Space: $freeSpaceGB GB"

    if ($freeSpaceGB -lt 64) {
        Write-Host "Warning: You need at least 64 GB of free disk space for the upgrade."
    } else {
        Write-Host "Disk space is sufficient for the upgrade."
    }
}
...
# Function to re-register COM components
function Register-COMComponents {
    Write-Host "Re-registering COM components (if needed)..."
...
Write-Host "Starting Windows 11 upgrade preparation..."
...
Register-COMComponents

Write-Host "System preparation for Windows 11 upgrade is complete!"


# -----------------------------
# STEP 1 — DUPLICATE SID CLEANUP
# -----------------------------
...
# -----------------------------
# STEP 2 — ADVANCED SID REPAIR
# -----------------------------
Write-Log "Performing advanced SID repair..."
...
foreach ($Key in $ProfileKeys) {
    ...
    try {
        ...
        Write-Log "Removing corrupted SID entry: $SID"
    ...
    if (-not (Test-Path $Path)) {
        Write-Log "Orphaned SID (folder missing): $SID"
        Remove-Item $Key.PSPath -Recurse -Force
        continue
    }
    ...
    if (-not (Test-Path "$Path\NTUSER.DAT")) {
        Write-Log "Corrupt profile (missing NTUSER.DAT): $SID"
        Remove-Item $Key.PSPath -Recurse -Force
        continue
    }
    ...
    if ($AllUsers.SID -notcontains $SID) {
        Write-Log "Orphaned profile (no matching user account): $SID"
        Remove-Item $Key.PSPath -Recurse -Force
        continue
    }
